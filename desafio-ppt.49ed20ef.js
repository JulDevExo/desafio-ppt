const e=new class{constructor(){this.routes=[],this.rootElement=null,window.addEventListener("popstate",()=>{this.render()})}setRootElement(e){this.rootElement=e}addRoute(e,t){this.routes.push({path:e,component:t})}navigate(e){window.history.pushState({},"",e),this.render()}render(){if(!this.rootElement)return;let e=window.location.pathname,t=this.routes.find(t=>t.path===e)||this.routes[0];if(t){this.rootElement.innerHTML="";let e=t.component();this.rootElement.appendChild(e)}}init(){"/"===window.location.pathname?this.navigate("/welcome"):this.render()}};var t={};const s=new URL(t=import.meta.resolve("5duda")).href,a=new URL(import.meta.resolve("9flAv")).href;var i={};const o=new URL(i=import.meta.resolve("iUMzC")).href;var n={};const r=new URL(n=import.meta.resolve("jmcSx")).href;var l={};const c=new URL(l=import.meta.resolve("6lVGu")).href;function d(){let t=document.createElement("div");return t.className="page welcome-page",t.innerHTML=`
  ...
  `;let s=t.querySelectorAll(".choice-btn");return s.forEach(a=>{a.addEventListener("click",()=>{let i=a.getAttribute("data-choice");s.forEach(e=>e.disabled=!0),a.classList.add("selected"),m.playGame(i);let o=document.createElement("div");o.className="countdown-overlay";let n=document.createElement("div");n.className="countdown-number countdown-animate",n.textContent="3",o.appendChild(n),t.appendChild(o);let r=3,l=setInterval(()=>{--r>0?(n.textContent=r.toString(),n.style.animation="none",n.offsetWidth,n.style.animation=""):(clearInterval(l),e.navigate("/result"))},1e3)})}),t}const w=new URL(import.meta.resolve("bquyp")).href,f=new URL(import.meta.resolve("idnrC")).href,L=new URL(n).href,E=new URL(l).href,S=new URL(i).href,y=new URL(import.meta.resolve("5uzWc")).href;function R(){let t=document.createElement("div");t.className="page result-page";let{playerChoice:s,computerChoice:a,result:i}=m.getState(),o="win"===i?w:"lose"===i?f:"",n={piedra:L,papel:E,tijera:S};t.innerHTML=`
    <div class="result-content">
      ...
      <div class="choices-display">
        <div class="choice-item">
          <h3>T\xfa</h3>
          <img src="${n[s||"piedra"]}" alt="${s}" class="choice-img" />
        </div>
        
        <div class="choice-item">
          <h3>Computadora</h3>
          <img src="${n[a||"piedra"]}" alt="${a}" class="choice-img" />
        </div>
      </div>
  ...
  `;let r=t.querySelector(".play-again-btn"),l=t.querySelector(".score-btn");return r.addEventListener("click",()=>{m.resetGame(),e.navigate("/game")}),l.addEventListener("click",()=>{e.navigate("/score")}),t}function $(){let t=document.createElement("div");t.className="page score-page";let s=m.getState();t.innerHTML=`
  ...
  `;let a=t.querySelector(".back-btn"),i=t.querySelector(".reset-btn");return a.addEventListener("click",()=>{e.navigate("/game")}),i.addEventListener("click",()=>{confirm("¿Estás seguro de que quieres reiniciar la puntuación?")&&(m.resetScore(),e.navigate("/score"))}),t}document.addEventListener("DOMContentLoaded",()=>{let t=document.getElementById("root");if(!t)return void console.error("Elemento #root no encontrado");e.setRootElement(t),e.addRoute("/welcome",d),e.addRoute("/game",b),e.addRoute("/result",R),e.addRoute("/score",$),e.init(),console.log("Aplicación Piedra, Papel o Tijera cargada")});
//# sourceMappingURL=desafio-ppt.49ed20ef.js.map
